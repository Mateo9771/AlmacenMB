INSERT INTO posiciones_almacen (nave, calle, posicion, altura, ubicaciones_concat, estado)
SELECT 
    nave, 
    calle, 
    posicion, 
    altura, 
    CONCAT(nave, '-', calle, '-', posicion, '-', altura) AS ubicaciones_concat, 
    'vacío' AS estado
FROM (
    SELECT naves.nave, calles.calle, posiciones.posicion, alturas.altura
    FROM 
        (SELECT 1 AS nave) naves,
        (SELECT 1 AS calle UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 
         UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10) calles,
        (SELECT 1 AS posicion UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5
         ...
         UNION ALL SELECT 156) posiciones,
        (SELECT 1 AS altura UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 
         UNION ALL SELECT 6) alturas
) combinaciones;
